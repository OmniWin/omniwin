...
contract Omniwin is ReentrancyGuard, VRFConsumerBaseV2, CCIPReceiver {
    ...
    // Mapping: chainSelector => totalAmountRaised
    mapping(uint64 => uint256) public amountRaisedPerChain;
    ...
    function buyEntry(
        ...
        amountRaisedPerChain[mainChainSelector] += _usdcAmount;
    ...
    function transferPrizesAndFunds(
        ...
        bool payoutSent = IERC20(usdcContractAddress).transfer(
            destinationWallet,
            amountForSeller
        );

        console.log("payoutSent: %s", payoutSent);

        if (!payoutSent) {
            revert FailedToSendPayoutToSeller();
        }

        console.log("raffle winner: %s", raffle.winner);
    ...
    function sendPrizeDistributionMessages(uint256 _raffleId) internal {
        for (uint256 i = 0; i < chainSelectors.length; ++i) {
            if (amountRaisedPerChain[chainSelectors[i]] > 0) {
    ...
    function requestRandomNumberFromVRF(
        ...
        //set dynamic gas limit depeding on how many chains the message will be sent
    ...
    /// @param _raffleId Id of the raffle
    /// @dev it triggers Chainlink VRF2 consumer, and generates a random number that is normalized and checked that corresponds to a OW player
    function setWinner(uint256 _raffleId) external nonReentrant {
        if (!isOwner()) revert NotTheOwner();

        EntryInfoStruct memory raffle = setWinnerActions(_raffleId);

        uint32 callbackGasLimit = 350_000;
    ...
    function _buyEntryCCIP(
        ...
        amountRaisedPerChain[_sourceChainSelector] += usdcAmount;
