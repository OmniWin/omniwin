...
contract Omniwin is ReentrancyGuard, VRFConsumerBaseV2, CCIPReceiver {
    ...
    event PrizeAwarded(uint256 indexed raffleId, address indexed winner);
    ...
    mapping(uint256 => mapping(address => ChainAction))
    ...
    mapping(uint256 => mapping(address => Claimed)) public hasClaimedRefund;
    ...
    function transferFeeToPlatform(uint256 _raffleId) internal nonReentrant {
    ...
    function claimRefundBoughtTickets(uint256 _raffleId) external nonReentrant {
        EntryInfoStruct storage entryInfo = rafflesEntryInfo[_raffleId];
        ...
        ) // Make sure source chain and sender are allowlisted
    {
        ...
        if (
            messageType == uint8(MESSAGE_TYPE.CREATE_RAFFLE_FROM_SIDECHAIN_SYN)
        ) {
            _createRaffleFromSidechain(message);
        }

        if (messageType == uint8(MESSAGE_TYPE.CREATE_RAFFLE_ACK)) {
            _enableRaffleFromSidechain(message);
        }
    ...
    function _enableRaffleFromSidechain(
        Client.Any2EVMMessage memory message
    ) internal {
        bytes32 messageIdSourceChain = message.messageId;
        address sender = abi.decode(message.sender, (address));

        (uint8 messageTypeReceived, uint256 raffleId) = abi.decode(
            message.data,
            (uint8, uint256)
        );
        ...
        emit RaffleEnabledFromSidechain(raffleId, sender, messageIdSourceChain);
    }
    ...
    function _createRaffleFromSidechain(
        ...
        bytes memory data = abi.encode(
            MESSAGE_TYPE.CREATE_RAFFLE_SYN_ACK,
            messageIdSourceChain,
            raffleId,
            raffles[raffleId].deadline,
            gasLimitAck
        );

        raffleChainActions[raffles.length - 1][_sender].hasActivity = true;

        // console.log("create raffle from sidechain");
        //Send syn to sidechain for raffle creation
        bytes32 messageId = sendMessage(
            message.sourceChainSelector,
            _sender,
            data,
