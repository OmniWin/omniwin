...
contract Omniwin is ReentrancyGuard, VRFConsumerBaseV2, CCIPReceiver {
    ...
    function sendPrizeDistributionMessages(
        ...
        RaffleInfo memory raffleInfo = chainlinkRaffleInfo[_raffleId];

        if (
            raffleChainActions[raffleInfo.id][address(this)].hasActivity &&
            raffleChainActions[raffleInfo.id][address(this)].amount > 0 &&
            !fundingList[_raffleId].platformFeeCollected
        ) {
            transferFeeToPlatform(raffleInfo.id);
    ...
    function _createRaffleFromSidechain(
        ...
        //Send ack to sidechain for raffle creation
        bytes32 messageId = sendMessage(
            message.sourceChainSelector,
            _sender,
            data,
