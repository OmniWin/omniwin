...
contract Omniwin is AccessControl, ReentrancyGuard, VRFConsumerBase {
    ...
    function createRaffle(
        ...
        uint48 _commissionInBasicPoints = 5000; //TODO:// get this from a global, and make a setter
        ...
        setupPriceStructures(idRaffle, _prices);
    ...
    function setupPriceStructures(
        uint256 idRaffle,
        PriceStructure[] calldata _prices
    ) internal {
        for (uint256 i = 0; i < _prices.length; ++i) {
            require(_prices[i].numEntries != 0, "numEntries is 0");
            pricesList[_prices[i].id] = PriceStructure({
                id: idRaffle,
    ...
    function claimRefund(uint256 _raffleId) external nonReentrant {
        ...
        console.log("EntryInfo status: %s", uint(entryInfo.status));
    ...
    function calculateRefund(
        ...
        uint48 entriesLength = rafflesEntryInfo[_raffleId].entriesLength;
        for (uint48 i = 0; i < entriesLength; i++) {
            if (entriesList[_raffleId][i].player == _player) {
                refundAmount += pricesList[_raffleId].price;
            }
        }
        return refundAmount;
    }
    ...
    function reclaimAssets(uint256 _raffleId) external nonReentrant {
        ...
        // Handle other asset types accordingly
