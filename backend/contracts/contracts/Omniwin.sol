...
contract Omniwin is AccessControl, ReentrancyGuard, VRFConsumerBase {
    ...
    uint256 maxDeadlineDuration = 60 days;
    ...
    struct RaffleStruct {
        ...
        ASSET_TYPE assetType; // type of the asset. Can be ERC20, ERC721, ERC1155, ETH
    ...
    function handleAssetTransferAndValidation(
        ...
        } else if (_assetType == ASSET_TYPE.ERC1155) {
            IERC1155 prizeToken = IERC1155(_prizeAddress);
            require(
                prizeToken.balanceOf(msg.sender, _prizeNumber) > 0,
                "Not the NFT owner"
            );
            require(
                prizeToken.isApprovedForAll(msg.sender, address(this)),
    ...
    function setFee(uint48 _fee) external onlyRole(OPERATOR_ROLE) {
    ...
    function claimRefund(uint256 _raffleId) external nonReentrant {
        ...
        require(
            ...
            "Raffle deadline has not passed"
        );
        require(
            entryInfo.amountRaised < fundingList[_raffleId].minimumFundsInWeis,
            "Minimum funding goal was met"
        );
        require(!hasClaimedRefund[_raffleId][msg.sender], "Already claimed");

        // Ensure there's a check here to prevent out-of-bounds access
        if (entriesList[_raffleId].length == 0) {
            console.log("No entries found for this raffle.");
            return;
        }
    ...
    function reclaimAssets(uint256 _raffleId) external nonReentrant {
        ...
        require(msg.sender == raffle.seller, "Only the seller can reclaim");
        require(
        ...
        require(
            rafflesEntryInfo[_raffleId].amountRaised <
                fundingList[_raffleId].minimumFundsInWeis,
            "Minimum funding goal was met"
        );
        ...
        if (raffle.assetType == ASSET_TYPE.ERC20) {
        ...
        } else if (raffle.assetType == ASSET_TYPE.ERC1155) {
            IERC1155(raffle.prizeAddress).safeTransferFrom(
                address(this),
                msg.sender,
                raffle.prizeNumber,
                1,
            ...
            require(sent, "Failed to send ETH prize");
        }
    ...
    function buyEntry(uint256 _raffleId, uint48 _id) external payable {
    ...
    function transferPrizesAndFunds(
        ...
        if (raffle.assetType == ASSET_TYPE.ERC20) {
            ...
            require(
                erc20Token.transfer(raffle.winner, raffle.prizeNumber),
                "Failed to send ERC20 prize"
            );
        } else if (raffle.assetType == ASSET_TYPE.ERC721) {
            ...
            IERC1155 erc1155Token = IERC1155(raffle.prizeAddress);
            ...
            require(success, "Failed to send ETH prize");
        }
        ...
        require(payoutSent, "Failed to send payout to seller");
    ...
    //TODO: add type of refund (ETH, ERC20, ERC721, ERC1155)
