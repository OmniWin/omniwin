...
export class UserController {
  ...
  public static async create(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const referralService = await new ReferralService(
        req.server as FastifyInstance,
      );
      const userService = await new UserService(req.server as FastifyInstance);
      ...
      for await (const part of parts) {
        ...
        if (part.type === "file") {
          ...
          const uploadDir = path.join(
            __dirname,
            "../../../../website/public/images/uploads/avatars/",
          );
  ...
  public static async fetchOpenseaNFTs(req: FastifyRequest, res: FastifyReply) {
    try {
      // TODO: Redo checks
      //   const { address } = req.user as any;
      const { chain } = req.query as { chain: "ethereum" | "matic" | "bsc" };
      ...
      sdk.auth("5659d0043fb54b588154c0d2734979f9");
      const nfts = await sdk.list_nfts_by_account({
        chain: chain,
        address: "0xE0bb5f1167Bd3e7D7986459c19261C0aFa2db41e",
      });

      return res.code(200).send({
        success: true,
        data: nfts?.data?.nfts ?? [],
        message: "Opensea NFTs fetched successfully",
      });
    } catch (error: any) {
      console.log(error);
      throw new HttpError(req.server, error.message);
