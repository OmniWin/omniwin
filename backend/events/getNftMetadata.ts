...
export async function getNFTData(nftContractAddress: string, tokenId: string, assetType: ASSET_TYPE, provider: ethers.JsonRpcProvider, chainId: number) {
    const nftContract = assetType === ASSET_TYPE.ERC721 ? new ethers.Contract(nftContractAddress, NFT_721_ABI, provider) : new ethers.Contract(nftContractAddress, NFT_1155_ABI, provider);
    ...
    try {
        switch (assetType) {
            case ASSET_TYPE.ERC721:
                ...
                response = await fetchNFTData(tokenURI, tokenId,chainId) as any;
...
async function downloadImage(url: string, tokenId: string, chainId: number) {
    ...
    if (url.startsWith("ipfs:")) {
        ...
        const imageName = `${tokenId}_${chainId}`;
...
async function fetchNFTData(uri: string, tokenId: string, chainId:number) {
    try {
        ...
        if (uri.startsWith('ipfs://')) {
            ...
            const image_local = await downloadImage(imageUri, tokenId, chainId);
            ...
            const image_local = await downloadImage(data.image, tokenId,chainId);
            if (image_local) data.image_local = image_local;
