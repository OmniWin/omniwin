...
const challenges = [
    {
        ...
        href: "#",
        description: "Begin your Adventure today! Where your interactions lead to rewards! Link up, follow, tweet, and show your Omniwin love. Each task is a step towards unlocking the full Omniwin experience. Ready for the challenge?",
        icon: PlayIcon,
        completed: true,
        currentStep: 6,
        totalSteps: 6,
                        ...
                        <div key={challenge.name} className="flex flex-col rounded-2xl border border-zinc-800 bg-gradient-to-tl from-zinc-900 to-zinc-800/30 shadow-xl hover:bg-zinc-800/50 group transition-all ease-in-out duration-300">
                            <div className="relative flex-1 px-6 pb-8 pt-16 md:px-8">
                                ...
                                <h3 className="text-xl font-medium text-zinc-50 flex items-center gap-2">
                                    {challenge.name}
                                    {challenge.completed && <span className="inline-flex items-center rounded-md bg-emerald-500/10 px-2 py-1 text-xs font-medium text-emerald-400 ring-1 ring-inset ring-emerald-500/20">Completed</span>}
                                </h3>
                                <p className="mt-4 text-base text-zinc-300 line-clamp-2" title={challenge.description}>{challenge.description}</p>
                                {
                                    challenge.currentStep > 0 && <>
                                        <div className="mt-8">
                                            <ProgressBar value={challenge.currentStep} maxValue={challenge.totalSteps} size="small" color="emerald-400" labelOutside labelOutsideText={challenge.totalSteps-challenge.currentStep != 0 ? `${challenge.totalSteps-challenge.currentStep} more step${challenge.totalSteps-challenge.currentStep !== 1 ? 's' : ''} to go` : 'Completed'} />
                                        </div>
                                    </>
                                }
                            </div>
                            <div className="rounded-bl-2xl rounded-br-2xl bg-zinc-800/30 p-6 md:px-8">
                                {!challenge.completed && !(challenge.currentStep > 0) && <a href={challenge.href} className="text-base font-medium text-emerald-400 hover:text-emerald-500 group/action">
                                    Start challenge
                                    <span className="group-hover/action:translate-x-1 transition-all ease-in-out duration-300 inline-block ml-2" aria-hidden="true"> &rarr;</span>
                                </a>}
