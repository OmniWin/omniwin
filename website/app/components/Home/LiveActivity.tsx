...
// export default function LiveActivity({ initialActivity }: { initialActivity: RaffleActivity[] }) {
export default function LiveActivity() {
    ...
    //     ...result
    // } = useInfiniteQuery({
    //     queryKey: ['liveActivity'],
    //     queryFn: ({ pageParam }) => fetchRaffleActivity('10', pageParam),
    //     //@ts-ignore
    ...
    const handleEvent = (
        ...
        for (let i = 0; i < eventData.length; i++) {
            ...
            queryClient.setQueryData(["liveActivity"], (oldData: RaffleParticipantsResponse["data"] | undefined) => {
                ...
                return {
                    ...oldData,
                    items: [...updatedItems, ...oldData.items],
                };
            });
        }
    };
            ...
            <div className="border border-zinc-800 rounded-xl overflow-auto max-h-80">
                <table className="w-full whitespace-nowrap text-left">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {result.map((participant, participantIndex) => {
                            // Check if the current participant is the last item in the last page
                            return (
                                <tr key={`${participant}-${participantIndex}`} className="hover:bg-zinc-800">
